...
test('GET /status/:jobId/stream 404s for a malformed jobId', async () => {
  ...
  try {
    const res = await fetch(`${baseUrl}/status/not-a-uuid/stream`);
...
test('GET /status/:jobId/stream 404s for a jobId that does not exist', async () => {
  ...
  try {
    const res = await fetch(`${baseUrl}/status/00000000-0000-4000-8000-000000000000/stream`);
...
test('GET /status/:jobId/stream sends an SSE event announcing job completion', async () => {
  ...
  try {
    ...
    const extractRes = await fetch(`${baseUrl}/extract`, { method: 'POST', body: form });
    const { jobId } = (await extractRes.json()) as { jobId: string };

    const streamRes = await fetch(`${baseUrl}/status/${jobId}/stream`);
